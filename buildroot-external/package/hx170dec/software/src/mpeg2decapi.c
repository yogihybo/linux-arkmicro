...
#define MPEG2DEC_BUILD_MINOR 140
    ...
    else
    {
        pDecInfo->outputFormat =
            GetDecRegister(DEC_REGS,
                           HWIF_DEC_OUT_TILED_E) ? MPEG2DEC_TILED_YUV420 :
        ...
        if(API_STOR.DecStat != HW_PIC_STARTED)
        {
            ...
            case DEC_PIC_HDR_RDY:
                    ...
                    pDecCont->StrmStorage.intraFreeze))
                {
                    ...
                    {
                        MPEG2_API_TRC("Mpeg2DecDecode# SKIP B\n");
                    }
        ...
        else
        {
                    ...
                    pDecCont->Hdrs.interlaced)
            {
                   ...
                   pDecCont->StrmStorage.outBuf[1])
                {
                    ...
                    if(MPEG2DEC_IS_FIELD_OUTPUT)
                        Mpeg2DecPrepareFieldProcessing(pDecCont);
                }
            }
...
static u32 Mpeg2SetRegs(DecContainer * pDecContainer, u32 strmBusAddress)
{
    ...
    SetDecRegister(pDecContainer->mpeg2Regs, HWIF_WRITE_MVS_E, pDecContainer->Hdrs.concealmentMotionVectors);   /* ??? */
...
static void Mpeg2DecPrepareFieldProcessing(DecContainer * pDecCont)
{
    ...
    {
        pDecCont->ApiStorage.bufferForPp = 1+pDecCont->StrmStorage.work0;
    }
    else
    {
        ASSERT(0);
