...
#define JPG_INFO  PTR_JPGC->info
    ...
    if(!PTR_JPGC->info.streamEndFlag)
    {
        ...
        {
            JPEGDEC_API_TRC("JpegDecDecode# ERROR: pDecIn->bufferSize % 256) != 0");
            return (JPEGDEC_INVALID_INPUT_BUFFER_SIZE);
        }
        ...
        {
            JPEGDEC_API_TRC("JpegDecDecode# ERROR: Increase input buffer size!\n");
            return (JPEGDEC_INVALID_INPUT_BUFFER_SIZE);
        }
    }
        ...
        if(JpegDecGetByte(&(PTR_JPGC->stream)) == 0xFF)
        {
            ...
            case COM:
                ...
                if(currentBytes != 0)
                {
                    ...
                    PTR_JPGC->stream.pCurrPos += (((currentBytes * 8) - 16) / 8);
                }
                break;
            default:
                break;
