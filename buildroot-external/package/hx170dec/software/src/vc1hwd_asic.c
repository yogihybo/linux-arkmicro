...
void SetPp(decContainer_t *pDecCont)
{
    ...
    {
        nextBufferIndex = pDecCont->ppControl.bufferIndex;
    }
    ...
    /* Connect PP output buffer to decoder output buffer */
    {
        ...
        luma = pDecCont->storage.pPicBuf[pDecCont->storage.workOut].
                                data.busAddress;
    ...
    if (!pDecCont->ppControl.usePipeline)
    {
        ...
        if (pPic[wOut].field[0].type < 2)   /* I or P picture */
        {
            ppIdx = w0;
            if (pDecCont->storage.interlace)
            {
    	...
    	{
            /*
    	    pDecCont->ppControl.displayIndex = 0;
