...
static u32 DpbBufFree(dpbStorage_t *dpb, u32 i);
static u32 OutBufFree(dpbStorage_t *dpb, u32 i);
...
/* Code */
    ...
    if (DISPLAY_SMOOTHING && dpb->prevOutIdx != INVALID_MEM_IDX)
    {
        ...
        {
            printf("HEPHEP NYT MENEE PÄÄLLE\n");
            exit(112);
        }
    }
#endif
       ...
       (frameNum != ((dpb->prevRefFrameNum + 1) % dpb->maxFrameNum)))
    {
        ...
        if(dpb->numOut && !DISPLAY_SMOOTHING)
        {
            ...
            for(i = 0; i < dpb->numOut; i++)
            {
                ...
                if(dpb->outBuf[i].data == dpb->currentOut->data)
                {
 ...
 * decoded pictures */
u32 DpbBufFree(dpbStorage_t *dpb, u32 i)
{

    u32 idx = dpb->buffer[i].memIdx;
 ...
 * reference anymore */
u32 OutBufFree(dpbStorage_t *dpb, u32 i)
{
