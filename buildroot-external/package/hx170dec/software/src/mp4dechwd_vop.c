...
u32 StrmDec_DecodeVopHeader(DecContainer * pDecContainer)
{
	...
	if(pDecContainer->Hdrs.vopTimeIncrementResolution != 0)
	{
		i = StrmDec_NumBits(pDecContainer->Hdrs.vopTimeIncrementResolution - 1);
	}
	//Add by Sim 2016.03.28
	//The headers are not complete, trying to guess time_increment_bits
	else
	{
        for (i = 1; i < 16; i++)
        {
            if ((StrmDec_ShowBits(pDecContainer,i + 6) & 0x37) == 0x30)
            	break;
            else if ((StrmDec_ShowBits(pDecContainer, i + 5) & 0x1F) == 0x18)
                break;
        }	
		pDecContainer->Hdrs.vopTimeIncrementResolution = 1 << i;
	}
