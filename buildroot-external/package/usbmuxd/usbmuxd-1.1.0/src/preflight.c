...
static void np_callback(const char* notification, void* userdata)
{
	...
	if (strcmp(notification, "com.apple.mobile.lockdown.request_pair") == 0) {
		...
		//system("echo usertrusted > /tmp/iphone_notify");sync();
		system("rm -rf /tmp/iphone_notify");
		sync();
...
retry:
	...
	if (version_major >= 7) {
		...
		if (!is_device_paired) {printf("%s:%d\n", __func__, __LINE__);
		...
		//system("echo waitingfortrusting > /tmp/iphone_notify");sync();
		system("touch /tmp/iphone_notify");
		sync();
