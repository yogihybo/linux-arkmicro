...
enum ECScreenType
{
	...
	EC_CAR_SCREEN_HORIZONTIAL_ROUND,             ///<  horizontal screetype and round mirror
	EC_CAR_SCREEN_VERTICAL_ROUND,                 ///< vertical screen type and round mirror
...
struct ECRoundScreenMirrorPos {
	uint16_t    screenRadius;                   ///< round scrren radius
	int16_t    mirrorOffsetX;                  ///< the X-axis offset of the center of the mirror area relative to the center of the screen
	int16_t    mirrorOffsetY;                  ///< the Y-axis offset of the center of the mirror area relative to the center of the screen

	ECRoundScreenMirrorPos() : screenRadius(0), mirrorOffsetX(0), mirrorOffsetY(0) {}
};
...
struct ECMirrorConfig
{
	...
	ECRoundScreenMirrorPos roundMirrorPos;         ///< set mirror position of the round scrren. this is valid when screenDirection == EC_SCREEN_DIRECTION_*_ROUND.
	
	string      reserve;                         ///< reserve(must be zero clearing)

    ECMirrorConfig() : type(EC_VIDEO_TYPE_H264), width(0), height(0),
        quality(0), touchMode(0), capScreenMode(0), screenDirection(EC_SCREEN_DIRECTION_UNKNOWN), reserve("") {}
};
