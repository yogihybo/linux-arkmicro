...
public:
    ...
    QString outDeviceInfo(QString device);
#ifdef ENABLE_ARKN141_CAMERA
...
QString DeviceWatcherPrivate::outDeviceInfo(QString device)
{
    QString cmdString = "fdisk -l | grep ";
    QString cmdOut = "";
    char buf[1024] = {0};
    int fd[2];
    int backfd;
    ...
    cmdString = cmdString + device;
    system(cmdString.toLocal8Bit());
...
void DeviceWatcherPrivate::diskDeviceAdd(const QString &path)
{
    if (regExpUSBPath(path)) {
        if (m_USBDiskPath.isEmpty()
            ...
            QString temp = outDeviceInfo(path);
            if (temp.contains(QString("NTFS"))) {
                umount(USB_MOUNT_POINT);
                QString command = QString("mount -t exfat ") + path + " "+ mountUSBPath;
                ...
                {
                    umount(USB_MOUNT_POINT);
                    QString command = QString("mount -t ntfs-3g ") + path + " "+ mountUSBPath;
                    qDebug()<<"+++++command_usb+++++"<<command;
        ...
        if (m_SDDiskPath.isEmpty()
            ...
            if (temp.contains(QString("NTFS"))) {
                ...
                QString command = QString("mount -t exfat ") + path + " "+ mountSDPath;
                int ret = system(command.toLocal8Bit().constData());
                qDebug()<<"========ret=========="<<ret;
                if (0 == ret) {
                    m_SDDiskPath = path;
                    ...
                    umount(SD_MOUNT_POINT);
                    QString command = QString("mount -t ntfs-3g ") + path + " "+ mountSDPath;
                    qDebug()<<"+++++command_sd+++++"<<command;
                    system(command.toLocal8Bit().constData());
                    m_USBDiskPath = path;
                    usbDiskScan(mountSDPath);
                }
            }
            else
            {
