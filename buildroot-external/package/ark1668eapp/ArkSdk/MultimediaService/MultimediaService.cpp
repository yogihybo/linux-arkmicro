...
void MultimediaService::videoPlayerVisible(const bool flag)
{
    g_VideoPlayer->videoPlayerVisible(flag);
}
void MultimediaService::videoPlayerSetGeometry(int x,int y,int width,int height)
{
     g_VideoPlayer->videoPlayerSetGeometry(x,y,width,height);
}
...
{
    connectSignalAndSignalByNamesake(g_MusicPlayer, m_Parent, SIGNAL(onMusicPlayerFileArtist(const int,QStringList)));
    ...
    connectSignalAndSignalByNamesake(g_MusicPlayer, m_Parent, SIGNAL(onMusicPlayerExit()));
    //    connectSignalAndSignalByNamesake(g_MusicPlayer, m_Parent, SIGNAL(onMusicPlayerFilePath(const int, const QString &)));
    ...
    connectSignalAndSignalByNamesake(g_VideoPlayer, m_Parent, SIGNAL(onVideoPlayerVisible(const bool)));
    connectSignalAndSignalByNamesake(g_DeviceWatcher, m_Parent, SIGNAL(onDeviceWatcherStatus(const int, const int)));
    connectSignalAndSignalByNamesake(g_VideoPlayer, m_Parent, SIGNAL(onSliderTouchEnable(bool)));  //dll add 20190805
    connectSignalAndSignalByNamesake(g_VideoPlayer, m_Parent, SIGNAL(onVideoPlayerExit()));
}
