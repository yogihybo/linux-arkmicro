...
//    if(AS_Aux == g_Audio->getAudioSource())
...
{
    initializePrivate();
    //SaveCurrentMusicPlayStatusToFile(MPPS_Play);
...
void Multimedia::callFinishedSlot(QDBusPendingCallWatcher *call)
{
...
void Multimedia::videoPlayerPlayListViewIndex(const int type, const int index, const int x, const int y, const int width, const int height, const int millesimal)
{
    ...
    g_Setting->setDisplayMode(DISP_PLAYER);
    if (NULL != m_Private->m_MultimediaServiceProxy) {
...
void Multimedia::videoPlayerExit()
{
    ...
    if (NULL != m_Private->m_MultimediaServiceProxy) {
        ...
        reply.waitForFinished();
        if (reply.isError()) {
            qDebug() << "method call Multimedia::videoPlayerExit failed" << reply.error();
        }
    }
}

void Multimedia::videoPlayerVisible(const bool flag)
{
    initializePrivate();
    if (NULL != m_Private->m_MultimediaServiceProxy) {
        QDBusPendingReply<> reply = m_Private->m_MultimediaServiceProxy->videoPlayerVisible(flag);
        reply.waitForFinished();
        if (reply.isError()) {
            qDebug() << "method call Multimedia::videoPlayerVisible failed" << reply.error();
        }
    }
}

void Multimedia::videoPlayerSetPlayMode(const int mode)
{
    initializePrivate();
    emit onVideoPlayerPlayMode(mode);
    qDebug()<<Q_FUNC_INFO<<"line :"<<__LINE__<<"video_playermode ="<<mode;
}

void Multimedia::videoPlayerSetGeometry(int x,int y,int width,int height)
{
    initializePrivate();
    if (NULL != m_Private->m_MultimediaServiceProxy) {
        QDBusPendingReply<void> reply = m_Private->m_MultimediaServiceProxy->videoPlayerSetGeometry(x,y,width,height);
        reply.waitForFinished();
        if (reply.isError()) {
            qDebug() << "method call Multimedia::videoPlayerSetGeometry failed" << reply.error();
        }
    }
}
...
void Multimedia::onHolderChange(const AudioSource oldHolder, const AudioSource newHolder)
{
    ...
    case AS_Music: {
        ...
        case AS_Aux: {
            if(newHolder != AS_BluetoothPhone)
            {
                qDebug()<<"+++++++MPPS_Pause5555++++++";
...
void MultimediaPrivate::connectAllSlots()
{
    if (NULL != m_MultimediaServiceProxy) {
        ...
        connectSignalAndSignalByNamesake(m_MultimediaServiceProxy, m_Parent, ARKSENDER(onVideoPlayerVisible(bool)));
        ...
        connectSignalAndSignalByNamesake(m_MultimediaServiceProxy, m_Parent, ARKSENDER(onMusicPlayerFileArtist(int,QStringList)));
        connectSignalAndSignalByNamesake(m_MultimediaServiceProxy, m_Parent, ARKSENDER(onMusicPlayerExit()));
        connectSignalAndSignalByNamesake(m_MultimediaServiceProxy, m_Parent, ARKSENDER(onVideoPlayerExit()));
        connectSignalAndSlotByNamesake(g_Audio, m_Parent, ARKRECEIVER(onHolderChange(const int, const int)));
        connectSignalAndSignalByNamesake(m_MultimediaServiceProxy, m_Parent, ARKSENDER(onSliderTouchEnable(bool)));  // dll add 20190805
