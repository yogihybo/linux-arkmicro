...
{
    if (QProcess::Running == m_Private->m_VideoPlayer->state()) {
        m_Private->videoPlayerVisible(flag);
    }
}
                    ...
                    while (time.elapsed() <= 500) {
                        if (m_Private->m_VideoPlayer->canReadLine()) {
                            ...
                            if (output.contains(QString("Exiting... (Quit)"))) {
                                ...
                                m_Private->m_Parent->quitHanlder(output);
                            }
                        } else {
                            usleep(1 * 1000);
                        }
...
void VideoPlayer::onReadyReadStandardOutput()
{
    while (m_Private->m_VideoPlayer->canReadLine()) {
        quitHanlder(m_Private->m_VideoPlayer->readLine());
    }
}
...
void VideoPlayer::quitHanlder(const QString &output)
{
    ...
    if (output.contains(QString("ANS_LENGTH="))) {
        ...
        emit onVideoPlayerPlayStatus(m_Private->m_DiskType, VPPS_SeekFinish);
    }
    else if((output.contains(QString("Exiting... (Fatal error)"))))
    {
        emit onVideoPlayerPlayStatus(m_Private->m_DiskType,VPPS_PlayError);
    }
    else {
        if (output != QString("\n")) {
...
VideoPlayerPrivate::~VideoPlayerPrivate()
{
    if (QProcess::NotRunning != m_VideoPlayer->state()) {
        ...
        time.start();
        while (time.elapsed() <= 500) {
            if (m_VideoPlayer->canReadLine()) {
                QString output = m_VideoPlayer->readLine();
                if (output.contains(QString("Exiting... (Quit)"))) {
                    break;
                } else {
                    m_Parent->quitHanlder(output);
                }
            } else {
                usleep(1 * 1000);
            }
    ...
    if ((VideoPlayerPrivate::CT_Undefine == m_LastCommand)
        ...
        if (QProcess::NotRunning == m_VideoPlayer->state()) {
            ...
            arguments << QString(QString::number(m_Width)
                                 ...
                                 + QString("+")
                                 + QString::number(m_Y));
//            arguments << QString("-aspect_ratio");//modify by wandz at20200609
//            arguments << QString("5");
            arguments << QString("-vo");
            arguments << QString("customfb");
            arguments << QString("-ao");
            arguments << QString("alsa");
//            arguments << QString("-debug");//print mplayer debug log but it will slow down
//            arguments << QString("-cache");
//            arguments << QString("8192");
//            arguments << QString("-lavdopts");
//            arguments << QString("threads=4");
            if (0 != millesimal) {
        ...
        emit m_Parent->onSliderTouchEnable(true);
...
void VideoPlayerPrivate::createSDFileNamesXml()
{
    ...
    if (persistantIndex.isEmpty()) {
        ...
        videoMillesmial = MultimediaPersistent::getVideoMillesmial(DWT_SDDisk);
    }
...
void VideoPlayerPrivate::createUSBFileNamesXml()
{
    ...
    if (persistantIndex.isEmpty()) {
        ...
        videoMillesmial = MultimediaPersistent::getVideoMillesmial(DWT_USBDisk);
    }
    QString data = persistantIndex + QString("-") + QString::number(videoMillesmial);
...
{
    if (QProcess::Running == m_VideoPlayer->state()) {
        if (flag) {
            m_VideoPlayer->write("pausing_keep_force takevideo\n");
        } else {
            m_VideoPlayer->write("pausing_keep_force giveupvideo\n");
        }
        m_VideoPlayer->waitForBytesWritten();
    }
}
