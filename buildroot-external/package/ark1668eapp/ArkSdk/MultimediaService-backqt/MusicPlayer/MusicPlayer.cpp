...
void MusicPlayer::onDeviceWatcherStatus(const DeviceWatcherType type, const DeviceWatcherStatus status)
{
    if (DWT_SDDisk == type) {
        ...
        case DWS_Empty: {
            ...
            m_Private->m_SdFileArtistList.clear();
            break;
        }
        case DWS_Busy: {
            m_Private->m_SDPathList.clear();
        ...
        case DWS_Ready: {
            ...
            emit onMusicPlayerFileArtist(DWT_SDDisk,m_Private->m_SdFileArtistList);
            break;
        }
        case DWS_Remove: {
            m_Private->m_SDPathList.clear();
        ...
        case DWS_Empty: {
            ...
            m_Private->m_USBFileArtistList.clear();
            break;
        }
        case DWS_Busy: {
            m_Private->m_USBPathList.clear();
        ...
        case DWS_Ready: {
            ...
            emit onMusicPlayerFileArtist(DWT_USBDisk,m_Private->m_USBFileArtistList);
            break;
        }
        case DWS_Remove: {
            m_Private->m_USBPathList.clear();
...
void MusicPlayerPrivate::playMusicIndex(const DeviceWatcherType type, const int index, const int millesimal)
{
    ...
    if ((temp.size() > index)
        ...
        if (QProcess::NotRunning == m_MusicPlayer->state()) {
            QString cmd = QString("/usr/bin/mplayer");
            ...
            arguments << QString("-novideo");
            if (0 != millesimal) {
                arguments << QString("-ss");
                arguments << QString::number(millesimal);
            }
            usleep(250*1000);
            qDebug()<<"=========2222arguments=========="<<arguments;
        ...
        qDebug()<<"cmd:"<<cmd;//cmd: "loadfile "/media/udisk/music/BoA - Only One.mp3""
        ...
        MultimediaPersistent::setMusicMillesmial(m_DiskType, millesimal);
    }
}
...
void MusicPlayerPrivate::createSDFileNamesXml()
{
    ...
    QFileInfo fileInfo;
    int i;
    qDebug()<<"++++++m_SDPathList.size()+++++++++++"<<m_SDPathList.size();
...
//    emit onMusicPlayerPlayStatus(m_Private->m_DiskType, m_Private->m_PlayStatus);
//    QFileInfo fileInfo(m_Private->m_CurrentFilePath);
//    m_Private->m_MusicInformation.parserFilePath(m_Private->m_CurrentFilePath);
//    emit onMusicPlayerElapsedInformation(m_Private->m_ElapsedTime, m_Private->m_Millesimal);
//    emit onMusicPlayerID3TagChange(m_Private->m_DiskType,
//                                             m_Private->m_PlayIndex,
//                                             fileInfo.filePath(),
//                                             m_Private->m_MusicInformation.getTitle(),
//                                             m_Private->m_MusicInformation.getArtist(),
//                                             m_Private->m_MusicInformation.getAlbum(),
//                                             m_Private->m_EndTime);
//    MultimediaPersistent::setMusicPathInfo(m_Private->m_DiskType, fileInfo.filePath() + QString("/") + fileInfo.created().toString(QString("yyyyMMddhhmmss")) + fileInfo.lastModified().toString(QString("yyyyMMddhhmmss")) + QString::number(fileInfo.size()));
//    MultimediaPersistent::setMultimediaType(m_Private->m_DiskType, MT_Music);
//    MultimediaPersistent::setMusicMillesmial(m_Private->m_DiskType, 0);
}
