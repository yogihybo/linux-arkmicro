...
//计算文件大小 modify by wandz at 20190718
static qint64 CalcFileSize(const QString &filename)
{
    QFile file(filename);
...
//    qDebug() <<"filename :"<<filename<<"filesize ="<< file.size()/(qPow(1024,2))<<"M";
    file.close();
    return file.size()/(qPow(1024,2));
}

//判断图片的支持格式，进行区分 modify by wandz at 20190718
bool ImagesupportFormat(const QString &filename)
{
    bool flag = false;
    QString imagename = filename;
    imagename = imagename.toLower();
    if(imagename.contains(".jpg") || imagename.contains(".bmp") \
            || imagename.contains(".jpeg") || imagename.contains(".png") \
            || imagename.contains(".gif") || imagename.contains(".tiff") \
            || imagename.contains(".pcx") || imagename.contains(".ico") \
            || imagename.contains(".tif")){
        return !flag;
    }
    else {
       return flag;
        ...
        if (NULL != (dirptr = opendir(dirPath.toLocal8Bit().data()))) {
            while (NULL != (direntptr = readdir(dirptr))) {
                if ((NULL != direntptr->d_name)
                    ...
                    fullname = dirPath + QString::fromLocal8Bit(direntptr->d_name);
                    if (-1 != stat64(fullname.toLocal8Bit().data(), &statbuff)) {
                        if(ImagesupportFormat(fullname) && CalcFileSize(fullname) >2.8){//image format >3m not load,modify by wandz at20190718
                                                    fullname.clear();
                                                }
