...
bool DiskDetach::checkOutDevice(QString device)
{
    bool isvalid = true;
    ...
    cmdOut = QString(buf);
    qDebug()<<__PRETTY_FUNCTION__<<__LINE__<<cmdOut;
    if((cmdOut.contains("Extended"))||(cmdOut == "")||(cmdOut.contains("Win95 Ext'd")))
    {
        isvalid = false;
    }
    return isvalid;
...
{
    qDebug()<<__PRETTY_FUNCTION__ << device << action;
                ...
                if ((condition1) || (m_Private->m_USBPartition.length() > 0)
                    ...
                    for (int i = 1; i < 10; ++i) {
                        if (QFile::exists(device + QString::number(i))) {
                            if(checkOutDevice(device + QString::number(i)))
                            {
