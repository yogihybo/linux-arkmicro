    ...
    if [ ${valid[${set_nro}]} -eq -1 ]; then
        ...
        then
            ...
            then
                ...
                nice_bin="echo"
                encode > ${TESTDIR}/case_${case_nro}/run_h264.sh
                chmod 777 ${TESTDIR}/case_${case_nro}/run_h264.sh
            fi
...
random_case() {
    ...
    if [ ! -e $test_case_list_dir/run_${case_nro}.sh ]; then
        ...
        cp $test_case_list_dir/run_${case_nro}.sh run.sh
        chmod u+x run.sh

        startTimeCounter "$case_nro"

        # Run encoder
        ./run.sh > $TESTDIR/case_${case_nro}/case_${case_nro}.log
        echo "$?" > $TESTDIR/case_${case_nro}/run_status
        ...
        then
            ...
            cp run.sh ${TESTDIR}/case_${case_nro}/run_h264.sh
        fi
...
case "$1" in
    ...
    list)
    echo "Reading cases from file 'list'"
    list=`cat list`
    echo "Running cases: $list"
