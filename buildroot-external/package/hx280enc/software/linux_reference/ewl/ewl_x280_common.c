...
int MapAsicRegisters(hx280ewl_t * ewl)
{
	...
	/* map sys registers to user space */
    pSysRegs =
        (u32 *) mmap(0, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED,
                     ewl->fd_mem, 0x40408000);
    if(pSysRegs == MAP_FAILED)
    {
        PTRACE("EWLInit: Failed to mmap sys regs\n");
        return -1;
    }
...
i32 EWLRelease(const void *inst)
{
    ...
    if(enc->pSysReg != MAP_FAILED)
        munmap((void *) enc->pSysReg, 0x1000);
