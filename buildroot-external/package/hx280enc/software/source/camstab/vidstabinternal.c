...
void VSInitAsicCtrl(VideoStb * pVidStab)
{
    ...
    val->asicCfgReg = 
        ((VS8290_AXI_WRITE_ID & (255)) << 24) |
        ((VS8290_AXI_READ_ID & (255)) << 16) |
        ((VS8290_BURST_LENGTH & (63)) << 8) |
        ((VS8290_BURST_INCR_TYPE_ENABLED & (1)) << 6) |
        ((VS8290_BURST_DATA_DISCARD_ENABLED & (1)) << 5) |
        ((VS8290_ASIC_CLOCK_GATING_ENABLED & (1)) << 4);

    val->irqDisable = VS8290_IRQ_DISABLE;
...
------------------------------------------------------------------------------*/
void VSSetRegisterValue(u32 *regMirror, regName name, u32 value)
{
    ...
    regVal = regMirror[field->base/4];

    /* Create bitmask for the field */
    ...
    regVal = regVal & ~(mask << field->lsb);

    /* Put new value of field in register */
    regVal = regVal | ((value & mask) << field->lsb);

    regMirror[field->base/4] = regVal;
}
...
void VSSetupAsicAll(VideoStb * pVidStab)
{
    ...
    if (val->inputImageFormat < ASIC_INPUT_RGB565)      /* YUV input */
        regMirror[2] = val->asicCfgReg |
            ((VS8290_INPUT_SWAP_16_YUV & (1)) << 14) |
            ((VS8290_INPUT_SWAP_32_YUV & (1)) << 2) |
            (VS8290_INPUT_SWAP_8_YUV & (1));
    else if (val->inputImageFormat < ASIC_INPUT_RGB888) /* 16-bit RGB input */
        regMirror[2] = val->asicCfgReg |
            ((VS8290_INPUT_SWAP_16_RGB16 & (1)) << 14) |
            ((VS8290_INPUT_SWAP_32_RGB16 & (1)) << 2) |
            (VS8290_INPUT_SWAP_8_RGB16 & (1));
    else                                                /* 32-bit RGB input */
        regMirror[2] = val->asicCfgReg |
            ((VS8290_INPUT_SWAP_16_RGB32 & (1)) << 14) |
            ((VS8290_INPUT_SWAP_32_RGB32 & (1)) << 2) |
            (VS8290_INPUT_SWAP_8_RGB32 & (1));

    /* Input picture buffers */
    VSSetRegisterValue(regMirror, HEncBaseInLum, val->inputLumBase);

    /* Common control register, use INTRA mode */
    VSSetRegisterValue(regMirror, HEncWidth, val->mbsInRow);
    VSSetRegisterValue(regMirror, HEncHeight, val->mbsInCol);
    VSSetRegisterValue(regMirror, HEncPictureType, 1);

    /* PreP control */
    VSSetRegisterValue(regMirror, HEncLumOffset, val->inputLumaBaseOffset);
    VSSetRegisterValue(regMirror, HEncRowLength, val->pixelsOnRow);
    VSSetRegisterValue(regMirror, HEncXFill, val->xFill);
    VSSetRegisterValue(regMirror, HEncYFill, val->yFill);
    VSSetRegisterValue(regMirror, HEncInputFormat, val->inputImageFormat);

    VSSetRegisterValue(regMirror, HEncBaseNextLum, val->rwNextLumaBase);
    VSSetRegisterValue(regMirror, HEncStabMode, val->rwStabMode);

    VSSetRegisterValue(regMirror, HEncRGBCoeffA,
                            val->colorConversionCoeffA & mask_16b);
    VSSetRegisterValue(regMirror, HEncRGBCoeffB,
                            val->colorConversionCoeffB & mask_16b);
    VSSetRegisterValue(regMirror, HEncRGBCoeffC,
                            val->colorConversionCoeffC & mask_16b);
    VSSetRegisterValue(regMirror, HEncRGBCoeffE,
                            val->colorConversionCoeffE & mask_16b);
    VSSetRegisterValue(regMirror, HEncRGBCoeffF,
                            val->colorConversionCoeffF & mask_16b);

    VSSetRegisterValue(regMirror, HEncRMaskMSB, val->rMaskMsb & mask_5b);
    VSSetRegisterValue(regMirror, HEncGMaskMSB, val->gMaskMsb & mask_5b);
    VSSetRegisterValue(regMirror, HEncBMaskMSB, val->bMaskMsb & mask_5b);
