...
ifeq ($(BR2_PACKAGE_ANDROID_TOOLS_LOGCAT),y)
ANDROID_TOOLS_TARGETS += logcat
endif
...
define ANDROID_TOOLS_INSTALL_TARGET_CMDS
	...
	$(INSTALL) -D -m 0755 $(@D)/build-logcat/logwrapper $(TARGET_DIR)/usr/bin/logwrapper$(sep)
	$(INSTALL) -D -m 0755 $(@D)/build-logcat/liblogwrap.so $(TARGET_DIR)/usr/lib/liblogwrap.so$(sep)
	$(INSTALL) -D -m 0755 $(@D)/build-logcat/liblog.so $(TARGET_DIR)/usr/lib/liblog.so$(sep)
endef
