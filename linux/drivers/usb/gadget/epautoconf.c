...
struct usb_ep *usb_ep_autoconfig_ss_ex(
	struct usb_gadget		*gadget,
	struct usb_endpoint_descriptor	*desc,
	struct usb_ss_ep_comp_descriptor *ep_comp,
	uint8_t pre_ep_num
)
{
	...
	list_for_each_entry (ep, &gadget->ep_list, ep_list) {
		if (usb_gadget_ep_match_desc(gadget, ep, desc, ep_comp)) {
			if (isdigit(ep->name[2])) {
				u8 num = simple_strtoul(&ep->name[2], NULL, 10);
				if (num == (pre_ep_num & (~USB_DIR_IN))) {
					goto found_ep;
				}
			} 			
		}
	}
	printk("ep:%02x can not be assigned\n", pre_ep_num);
...
EXPORT_SYMBOL_GPL(usb_ep_autoconfig_ss_ex);
...
{
	return usb_ep_autoconfig_ss_ex(gadget, desc, NULL, pre_ep_num);
}
EXPORT_SYMBOL_GPL(usb_ep_autoconfig_ex);
