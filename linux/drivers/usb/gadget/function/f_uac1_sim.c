...
#if USE_AUDIO_MIC
static struct usb_interface_assoc_descriptor uac1_iad = {
	.bLength		= sizeof(uac1_iad),
...
static struct uac1_ac_header_descriptor_2 ac_header_desc = {
	...
	.bDescriptorSubtype =	UAC_HEADER,
	.bcdADC =		cpu_to_le16(0x0100),
	.wTotalLength =		cpu_to_le16(30),//UAC_DT_TOTAL_LENGTH
	.bInCollection =	F_AUDIO_AS_OUT_INTERFACE,
	.baInterfaceNr = {
	/* Interface number of the AudioStream interfaces */
...
#define FEATURE_UNIT_ID		0
#else
#define USB_IN_OT_ID	2
#define FEATURE_UNIT_ID		1
#endif
...
static struct uac_format_type_i_discrete_descriptor_9 as_in_type_i_desc = {
...
static struct usb_endpoint_descriptor as_in_ep_desc  = {
	...
	.bmAttributes =		USB_ENDPOINT_SYNC_NONE
				| USB_ENDPOINT_XFER_ISOC,
	.wMaxPacketSize	=	cpu_to_le16(0xc0),
	.bInterval =		4,
};
...
#if USE_AUDIO_MIC
	(struct usb_descriptor_header *)&uac1_iad,
#endif
...
f_audio_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	case 0xc0 :
		...
		if (ctrl->bRequest != 0x53)
			break;
		value = 4;
		put_unaligned_le32(result, req->buf);
		break;
	default:
...
static int f_audio_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (intf == uac1->as_in_intf) {
		...
		if (audio->in_ep == NULL) {
			ep = usb_ep_autoconfig(f->config->cdev->gadget, &as_in_ep_desc);
			if (!ep)
				return -EINVAL;
			audio->in_ep = ep;
...
#if EP_ASSIGN_AT_CONN
	usb_ep_autoconfig_release(uac1->g_audio.in_ep);
	uac1->g_audio.in_ep = NULL;
#endif
}
...
printk("%s:%d\n", __func__, __LINE__);
...
#else
	/* Set channel numbers */
	io_in_it_desc.bNrChannels = 2;
	as_in_type_i_desc.bNrChannels = 2;
	...
	memcpy(sam_freq, &rate, 3);
	rate = 22050;
	sam_freq = as_in_type_i_desc.tSamFreq[4];
	memcpy(sam_freq, &rate, 3);
	rate = 24000;
	sam_freq = as_in_type_i_desc.tSamFreq[5];
	memcpy(sam_freq, &rate, 3);
	rate = 32000;
	sam_freq = as_in_type_i_desc.tSamFreq[6];
	memcpy(sam_freq, &rate, 3);
	rate = 44100;
	sam_freq = as_in_type_i_desc.tSamFreq[7];
	memcpy(sam_freq, &rate, 3);
	rate = 48000;
	sam_freq = as_in_type_i_desc.tSamFreq[8];
	memcpy(sam_freq, &rate, 3);
#endif
	...
	if (status < 0)
		goto fail;
#if USE_AUDIO_MIC
	uac1_iad.bFirstInterface = status;
#endif
...
#if EP_ASSIGN_AT_CONN
	uac1->ep_addr_in = 0x81;
	as_in_ep_desc.bEndpointAddress = uac1->ep_addr_in;
	audio->in_ep = NULL;
#else
...
static bool apple_audio_req_match(struct usb_function *f,
					const struct usb_ctrlrequest *ctrl,
					bool config0)
{	
	switch (ctrl->bRequest) {
...
printk("%s:%d\n", __func__, __LINE__);
	...
	uac1->g_audio.func.req_match = apple_audio_req_match;
...
#else
DECLARE_USB_FUNCTION_INIT(audio_sim, f_audio_alloc_inst, f_audio_alloc);
#endif
MODULE_LICENSE("GPL");
