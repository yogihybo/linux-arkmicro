...
#define ADB_BULK_BUFFER_SIZE           16384
...
#define ADB_INTERFACE_STRING_INDEX	0
static struct usb_string adb_string_defs[] = {
	[ADB_INTERFACE_STRING_INDEX].s	= "ADB Interface",
	{  },	/* end of list */
};
...
static void adb_complete_out(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (req) {
		if (req->status != 0){//printk("+++%s:%d+++req->status=%d\n", __func__, __LINE__, req->status);
			...
			if (req->actual > 0)
				adb_req_put(dev, &dev->rx_used, req);
			else {//printk("+++%s:%d+++ zero len \n", __func__, __LINE__);
				adb_req_put(dev, &dev->rx_idle, req);
			}
		}
	}
...
static int adb_rx_submit(struct adb_dev *dev, struct usb_request *req)
{
	int ret, r;
				...
				size_t count, loff_t *pos)
{
	...
	req = NULL;
	ret = wait_event_interruptible(dev->read_wq, (req = adb_req_get(dev, &dev->rx_used)) != NULL);
	if (ret < 0) {
		r = ret;
		if (req)
	...
	if (!dev->error && req) {
		...
		if (count >= cur_actual) {
			adb_rx_submit(dev, req);//the req is read completely, return to the usb core
			dev->cur_read_pos = 0;
			while ((req = adb_req_get(dev, &dev->rx_idle))) {
				req->length = ADB_BULK_BUFFER_SIZE;
				adb_rx_submit(dev, req);
			}
		} else {
...
adb_function_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	req = NULL;
	while ((req = adb_req_get(dev, &dev->rx_used)))
		adb_request_free(req, dev->ep_out);
#endif
	req = NULL;
...
static void adb_function_suspend(struct usb_function *f)
{
	...
	printk("adb_function_suspend cdev %p\n", cdev);
	dev->online = 0;
	dev->error = 1;
	usb_ep_disable(dev->ep_in);
...
#endif
static int adb_setup(struct adb_dev *dev)
{
	int ret;
...
static void adb_cleanup(struct adb_dev *dev)
{
...
static struct f_adb_opts *to_adb_opts(struct config_item *item)
{
	return container_of(to_config_group(item), struct f_adb_opts,
		func_inst.group);
}

static void adb_attr_release(struct config_item *item)
{
	struct f_adb_opts *opts = to_adb_opts(item);
...
static void adb_free_inst(struct usb_function_instance *fi)
{
	...
	opts = container_of(fi, struct f_adb_opts, func_inst);
	if (NULL != opts->name)
		kfree(opts->name);
	adb_cleanup(opts->dev);
...
{
	struct f_adb_opts *opts = NULL;

	opts = kzalloc(sizeof(struct f_adb_opts), GFP_KERNEL);
	if (!opts)
