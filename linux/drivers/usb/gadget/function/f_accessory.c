...
static unsigned int acc_poll(struct file *fp, struct poll_table_struct *wait)
{
	struct acc_dev *dev = (struct acc_dev *)fp->private_data;
	unsigned int mask = 0;
//printk("dev->online = %d, dev->error = %d dev->rx_done = %d \n", dev->online, dev->error, dev->rx_done);
	...
	if (fp->f_mode & FMODE_READ && dev->online && dev->rx_done)
		mask |= POLLIN| POLLRDNORM;
	if (fp->f_mode & FMODE_WRITE && dev->online && (!list_empty_careful(&dev->tx_idle)))
...
*/
	...
	if (b_requestType == (USB_DIR_OUT | USB_TYPE_VENDOR)) {
		if (b_request == ACCESSORY_START) {
			dev->start_requested = 1;printk("%s:%d\n", __func__, __LINE__);
...
static void acc_start_work(struct work_struct *data)
{
	...
	struct acc_dev *dev = container_of(to_delayed_work(data),
						struct acc_dev, start_work);

	if (dev->online && dev->start_requested) {
		envp[0] = "ACCESSORY=START";
		printk("%s aoa start\n", __func__);
	} else if (dev->online) {
		envp[0] = "ACCESSORY=CONNECTED";
		printk("%s connected\n", __func__);
	} else {
		envp[0] = "ACCESSORY=DISCONNECTED";
		...
		unsigned intf, unsigned alt)
{
	...
	dev->online = 1;printk("%s:%d\n", __func__, __LINE__);
	dev->disconnected = 0;
	schedule_delayed_work(
				&dev->start_work, msecs_to_jiffies(2));
...
static void acc_function_disable(struct usb_function *f)
{
	...
	/* readers may be blocked waiting for us to go online */
	wake_up(&dev->read_wq);printk("%s:%d\n", __func__, __LINE__);

	VDBG(cdev, "%s disabled\n", dev->function.name);
}

static void acc_function_suspend(struct usb_function *f)
{
	struct acc_dev	*dev = func_to_dev(f);

	printk("%s:%d\n", __func__, __LINE__);
	acc_function_disable(f);
	schedule_delayed_work(
				&dev->start_work, msecs_to_jiffies(2));
	printk("%s:%d\n", __func__, __LINE__);
}
...
static int acc_bind_config(struct usb_configuration *c)
{
	...
	dev->function.suspend = acc_function_suspend;
