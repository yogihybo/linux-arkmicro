...
static struct usb_device *get_root_hub_usb_dev(struct usb_device	*pusb_dev)
{
	struct usb_device* parent = pusb_dev->parent;
	struct usb_device* pdev = parent;
	while(1) {
		if (NULL == parent) {
			parent = pusb_dev;
			break;
	...
	printk("%s----->usb dev name:%s dev:%p\n", __func__, dev_name(&parent->dev), &parent->dev);
	return parent;
}
...
int usb_stor_probe2(struct us_data *us)
{
	...
	if (1) {
		struct usb_device *tmp_dev = get_root_hub_usb_dev(us->pusb_dev);
		result = scsi_add_host(us_to_host(us), &tmp_dev->dev);
	}
#else
