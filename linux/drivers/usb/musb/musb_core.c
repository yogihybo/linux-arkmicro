 ...
 */
#if NICHOLAS_ADD
extern void musb_dma_channel_release(struct musb *musb);
#endif
 ...
 */
	...
	schedule_work(&musb->irq_work.work);
...
static void musb_irq_work(struct work_struct *data)
{
	...
	if(musb->reset_mode == MUSB_OTG) {
		...
		musb_root_disconnect(musb);
		if (musb->a_wait_bcon != 0 && (musb->port_mode == MUSB_OTG)/*is_otg_enabled(musb)*/)
			musb_platform_try_idle(musb, jiffies
				+ msecs_to_jiffies(musb->a_wait_bcon));

		printk(KERN_ALERT "[musb] %s: reset otg\n", __func__);
		spin_lock_irqsave(&musb->lock, flags);
...
static void musb_recovery_usb_proc(struct work_struct *work)
{
	...
	if (port1_status & USB_PORT_STAT_ENABLE) {
		printk(KERN_ALERT "[musb] %s: port already enabled, skipping "
...
static void musb_reset_timer_handler(struct timer_list *t)
{
	...
	printk(KERN_ALERT "[musb] %s: reset timer fired\n", __func__);
