...
static void musb_h_tx_flush_fifo(struct musb_hw_ep *ep)
{
	...
	while (csr & MUSB_TXCSR_FIFONOTEMPTY) {
		...
		if (dev_WARN_ONCE(musb->controller, retries-- < 1,
				...
				ep->epnum, csr)){
			dev_alert(musb->controller, "%s: host TX FIFONOTEMPTY csr: %02x\n", __func__, csr);
			return;
		}
		mdelay(1);
	}
...
musb_start_urb(struct musb *musb, int is_in, struct musb_qh *qh)
{
	...
	{
		dev_err(musb->controller, "%s: invalid epnum %d, exit\n", __func__, epnum);
		return;
	}
...
musb_host_packet_rx(struct musb *musb, struct urb *urb, u8 epnum, u8 iso_err)
{
	...
	if (rx_count > 512) {
		dev_alert(musb->controller, "%s: rx_count = %d\n", __func__, rx_count);
	}
...
#if NICHOLAS_ADD
void musb_dma_channel_release(struct musb *musb)
{
	...
	for(i=0; (i<musb->config->num_eps) && (i<MUSB_C_NUM_EPS); i++)
	{
		...
		if(hw_ep->rx_channel)
		{
			dma_controller->channel_release(hw_ep->rx_channel);
			hw_ep->rx_channel = NULL;
		}
		if(hw_ep->tx_channel)
		{
			dma_controller->channel_release(hw_ep->tx_channel);
			hw_ep->tx_channel = NULL;
		}
	}
}
...
/* FIXME if !epnum, do the same for RX ... */
		...
		if (dma_channel && musb_tx_dma_program(dma_controller,
					hw_ep, qh, urb, offset, load_count))
			load_count = 0;
#else
		if (can_bulk_split(musb, qh->type))
			load_count = min((u32) hw_ep->max_packet_sz_tx,
	...
	gfp_t				mem_flags)
{
	...
	default:
		 ...
		 */
#if NICHOLAS_ADD
		if(urb->dev->actconfig)
		{
			struct usb_interface *intf = urb->dev->actconfig->interface[0];
