...
static void cyttsp5_si_get_sensing_conf_data(struct cyttsp5_core_data *cd)
{
	...
	if(cd->cpdata && cd->cpdata->max_tch)
		scd->max_tch = cd->cpdata->max_tch;
	else
		scd->max_tch = scd_dev->max_num_of_tch_per_refresh_cycle;
#ifdef CYTTSP5_DEBUG
	printk(KERN_ALERT "### %s, dev_tree_max_tch:%d, reg_read_max_tch:%d, final_max_tch:%d.\n",
		__FUNCTION__, cd->cpdata->max_tch, scd_dev->max_num_of_tch_per_refresh_cycle, scd->max_tch);
#endif
