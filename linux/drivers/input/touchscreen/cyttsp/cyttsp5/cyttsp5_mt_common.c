...
static int cyttsp5_touch_release[CY_HID_MAX_REPORTS] = {0};
	...
	struct cyttsp5_touch *touch)
{
	...
	cyttsp5_touch_release[touch->abs[CY_TCH_T]] = 0;
...
#endif
#ifdef CYTTSP5_DEBUG
	printk(KERN_ALERT "### id:%d, x:%d, y:%d\n", touch->abs[CY_TCH_T], touch->abs[CY_TCH_X], touch->abs[CY_TCH_Y]);
#endif
}
		...
		struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		if (tch->abs[CY_TCH_E] == CY_EV_LIFTOFF) {
			//touch released
			cyttsp5_touch_release[t] = 1;
#ifdef CYTTSP5_DEBUG
			printk(KERN_ALERT "### id:%d, release\n", t);
#endif
...
static int cyttsp5_xy_worker(struct cyttsp5_mt_data *md)
{
	...
	if (num_cur_tch) {
		...
		//touch released
		//for(i=0; i<CY_HID_MAX_REPORTS; i++) {
		//	cyttsp5_touch_release[i] = 1;
		//}
#ifdef CYTTSP5_DEBUG
		printk(KERN_ALERT "### release all.\n");
#endif
	}
