...
static u32 rtl8188fs_hal_init(PADAPTER padapter)
{
	...
	*	rtw_hal_disable_interrupt(padapter); */
	...
	pHalData->current_channel = 6;
        ...
        RTW_INFO("%s:%d>>>>>>>>>>>>>>>>>>>>>>>>>>>> = %dms\n",__func__, __LINE__, rtw_get_passing_time_ms(fwdl_start_time));
#if (HAL_MAC_ENABLE == 1)
	...
	BBTurnOnBlock_8188F(padapter);
        //RTW_INFO("%s:%d>>>>>>>>>>>>>>>>>>>>>>>>>>>> = %dms\n",  __func__, __LINE__, rtw_get_passing_time_ms(fwdl_start_time));///0ms
	rtw_hal_set_chnl_bw(padapter, padapter->registrypriv.channel,
		CHANNEL_WIDTH_20, HAL_PRIME_CHNL_OFFSET_DONT_CARE, HAL_PRIME_CHNL_OFFSET_DONT_CARE);
        //RTW_INFO("%s:%d>>>>>>>>>>>>>>>>>>>>>>>>>>>> = %dms\n", __func__, __LINE__,  rtw_get_passing_time_ms(fwdl_start_time));///+10ms
	...
	*/
	...
	_RfPowerSave(padapter);
        //RTW_INFO("%s:%d>>>>>>>>>>>>>>>>>>>>>>>>>>>> = %dms\n", __func__, __LINE__,rtw_get_passing_time_ms(fwdl_start_time));///0ms
	rtl8188f_InitHalDm(padapter);////update 20190928
        RTW_INFO("%s:%d>>>>>>>>>>>>>>>>>>>>>>>>>>>> = %dms\n",__func__, __LINE__, rtw_get_passing_time_ms(fwdl_start_time));///+20ms
...
#endif /* #if (MP_DRIVER == 1) */
	{
		...
		if (pwrctrlpriv->rf_pwrstate == rf_on) {
			...
			halrf_lck_trigger(&pHalData->odmpriv);////update 20190928
                        RTW_INFO("%s:%d>>>>>>>>>>>>>>>>>>>>>>>>>>>> = %dms\n",__func__, __LINE__,  rtw_get_passing_time_ms(fwdl_start_time));
