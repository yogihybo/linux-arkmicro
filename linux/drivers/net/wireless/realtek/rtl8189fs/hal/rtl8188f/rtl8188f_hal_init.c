...
)
{
	...
	remainSize_p1 = buffSize % blockSize_p1;
        ...
        //RTW_INFO("_BlockWrite:1>>>>>>>>>>>>rtl8188f_FirmwareDownload = %dms\n", rtw_get_passing_time_ms(start_time));
        //RTW_INFO("blockCount_p1:1 = %d\n",blockCount_p1);
	for (i = 0; i < blockCount_p1; i++) {
                if(i && (i % 256 == 0)){
                        msleep(1);}//////////update 20190925
#ifdef CONFIG_USB_HCI
        ...
        //RTW_INFO("_BlockWrite:2>>>>>>>>>>>>rtl8188f_FirmwareDownload = %dms\n", rtw_get_passing_time_ms(start_time));
#ifdef CONFIG_PCI_HCI
...
)
{
	...
	remainSize = size % MAX_DLFW_PAGE_SIZE;
        start_time = rtw_get_current_time();
        //RTW_INFO("WriteFW:1>>>>>>>>>>>>rtl8188f_FirmwareDownload = %dms\n", rtw_get_passing_time_ms(start_time));
	for (page = 0; page < pageNums; page++) {
		offset = page * MAX_DLFW_PAGE_SIZE;
		ret = _PageWrite(padapter, page, bufferPtr + offset, MAX_DLFW_PAGE_SIZE);
                //RTW_INFO("WriteFW:2>>>>>>>>>>>>rtl8188f_FirmwareDownload = %dms\n", rtw_get_passing_time_ms(start_time));
		if (ret == _FAIL) {
			printk("====>%s %d\n", __func__, __LINE__);
			goto exit;
		}
	...
	if (remainSize) {
		...
		ret = _PageWrite(padapter, page, bufferPtr + offset, remainSize);
                //RTW_INFO("WriteFW:3>>>>>>>>>>>>rtl8188f_FirmwareDownload = %dms\n", rtw_get_passing_time_ms(start_time));
		if (ret == _FAIL) {
			printk("====>%s %d\n", __func__, __LINE__);
			goto exit;
		}
...
s32 rtl8188f_FirmwareDownload(PADAPTER padapter, BOOLEAN  bUsedWoWLANFw)
{
	...
	u8 write_fw = 0;
	systime fwdl_start_time = rtw_get_current_time();
	...
	*/
	...
	while (!RTW_CANNOT_IO(padapter)
		...
		rtw_write8(padapter, REG_MCUFWDL, rtw_read8(padapter, REG_MCUFWDL) | FWDL_ChkSum_rpt);
                //RTW_INFO("Test:22222++++++rtl8188f_FirmwareDownload = %dms\n", rtw_get_passing_time_ms(fwdl_start_time));
		rtStatus = _WriteFW(padapter, pFirmwareBuf, FirmwareLen);////
		if (rtStatus != _SUCCESS)
			continue;
                //RTW_INFO("Test:33333++++++rtl8188f_FirmwareDownload = %dms\n", rtw_get_passing_time_ms(fwdl_start_time));
...
DLFW_FAIL:
        //RTW_INFO("Test:44++++++rtl8188f_FirmwareDownload = %dms\n", rtw_get_passing_time_ms(fwdl_start_time));
	if (rtStatus == _FAIL) {
		/* Disable FWDL_EN */
...
exit:
	...
	RTW_INFO(" <=== %s()--------------------%dms\n", __func__,rtw_get_passing_time_ms(fwdl_start_time));
        //RTW_INFO("Test:55++++++rtl8188f_FirmwareDownload = %dms\n", rtw_get_passing_time_ms(fwdl_start_time));
	return rtStatus;
}
...
static void hw_var_set_opmode(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
        systime start_time;
        start_time = rtw_get_current_time();
        printk("====>%s ---%d ---time:%dms\n", __func__, __LINE__, rtw_get_passing_time_ms(start_time));
	if (isMonitor == _TRUE) {
