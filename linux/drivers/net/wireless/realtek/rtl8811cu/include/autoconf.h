...
/* CONFIG_SDIO_INDIRECT_ACCESS / DBG_SDIO_INDIRECT_ACCESS: SDIO-only, not
 * defined here (this is the USB build) -- two call sites in ioctl_linux.c
 * (rtw_mp_sd_iread/rtw_mp_sd_iwrite) guard on CONFIG_SDIO_INDIRECT_ACCESS
 * alone, without also checking CONFIG_SDIO_HCI, so defining it for a USB
 * build pulls in calls to rtw_sd_iread8/16/32 that don't exist outside the
 * SDIO HAL. */
