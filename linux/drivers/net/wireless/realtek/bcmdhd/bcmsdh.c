...
{
	bcmsdh_info_t *bcmsdh = (bcmsdh_info_t *)sdh;
//	return sdioh_iovar_op(bcmsdh->sdioh, name, params, plen, arg, len, set);//add 20210511
}
...
bcmsdh_intr_query(void *sdh)
{
	...
	ASSERT(bcmsdh);
//	status = sdioh_interrupt_query(bcmsdh->sdioh, &on);//add 20210511
...
//	status = sdioh_interrupt_set(bcmsdh->sdioh, TRUE);//add 20210511
#ifdef BCMSPI_ANDROID
...
//	status = sdioh_interrupt_set(bcmsdh->sdioh, FALSE);//add 20210511
#ifdef BCMSPI_ANDROID
	data = bcmsdh_cfg_read_word(sdh, 0, 4, NULL);
	data &= ~0xE0E70000;
	bcmsdh_cfg_write_word(sdh, 0, 4, data, NULL);
...
//	status = sdioh_interrupt_register(bcmsdh->sdioh, fn, argh);//add 20210511
	return (SDIOH_API_SUCCESS(status) ? 0 : BCME_ERROR);
}
...
//	status = sdioh_interrupt_deregister(bcmsdh->sdioh);//add 20210511
	return (SDIOH_API_SUCCESS(status) ? 0 : BCME_ERROR);
}
...
bcmsdh_cfg_read(void *sdh, uint fnc_num, uint32 addr, int *err)
{
	...
	do {
		if (retry)	/* wait for 1 ms till bus get settled down */
			OSL_DELAY(1000);
#endif // endif
//	status = sdioh_cfg_read(bcmsdh->sdioh, fnc_num, addr, (uint8 *)&data);//add 20210511
#ifdef SDIOH_API_ACCESS_RETRY_LIMIT
...
bcmsdh_cfg_write(void *sdh, uint fnc_num, uint32 addr, uint8 data, int *err)
{
	...
	do {
		if (retry)	/* wait for 1 ms till bus get settled down */
			OSL_DELAY(1000);
#endif // endif
//	status = sdioh_cfg_write(bcmsdh->sdioh, fnc_num, addr, (uint8 *)&data);//add 20210511
#ifdef SDIOH_API_ACCESS_RETRY_LIMIT
	} while (!SDIOH_API_SUCCESS(status) && (retry++ < SDIOH_API_ACCESS_RETRY_LIMIT));
#endif // endif
	if (err)
...
//	status = sdioh_cis_read(bcmsdh->sdioh, func, cis, length);//add 20210511
...
//		SDIOH_READ, SDIO_FUNC_1, addr, &word, size);//add 20210511
...
//	                              SDIOH_READ, fn, addr, width, nbytes, buf, pkt);//add 20210511
...
//	status = sdioh_request_buffer(bcmsdh->sdioh, SDIOH_DATA_PIO, incr_fix,
//	                              SDIOH_WRITE, fn, addr, width, nbytes, buf, pkt);//add 20210511
...
//	                              addr, 4, nbytes, buf, NULL);//add 20210511
