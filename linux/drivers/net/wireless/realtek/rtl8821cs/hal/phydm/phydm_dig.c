...
static void pre_phydm_tdma_dig_cbk(struct timer_list *t)
{
	struct dm_struct *dm = from_timer(dm, t, tdma_dig_timer);
...
void phydm_tdma_dig_timers_usb(void *dm_void, u8 state)
{
	...
	if (state == INIT_TDMA_DIG_TIMMER) {
		...
		timer_setup(&dm->tdma_dig_timer, pre_phydm_tdma_dig_cbk, 0);
