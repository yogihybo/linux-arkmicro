...
#ifdef CONFIG_STORAGE_BIND_PORT
	if (1) {
		printk("#%s: dev name:%s\n", __func__, dev_name(sdp->host->dma_dev));
		char dev_disk_name[32] = {0};
		sprintf(dev_disk_name, "sd%s", dev_name(sdp->host->dma_dev));
		error = sd_format_disk_name(dev_disk_name, index, gd->disk_name, DISK_NAME_LEN);
	}
#else
