...
#ifdef CONFIG_SOC_ARKN141
#define INTEGRATOR_LOGIC_MODULE0_BASE   0x70078000
#endif

#ifdef CONFIG_SOC_ARKN141S
#define INTEGRATOR_LOGIC_MODULE0_BASE   0x48003000
#endif
...
#define CODEC_ENCODER_INT		        (1 + 16)

#define VP_PB_INT_LT                    CODEC_ENCODER_INT
/*
...
static struct clk *hx280enc_clk = NULL;
...
static unsigned int hx280enc_poll(struct file *filp, poll_table *wait)
{
	hx280enc_t *dev = (hx280enc_t *) filp->private_data;
	unsigned int mask = 0;

	poll_wait(filp, &dev->waitq, wait);
	if (hx280enc_data.hw_ready) {
		mask |= POLLIN | POLLRDNORM;
		hx280enc_data.hw_ready = 0;
	}
...
int __init hx280enc_init(void)
{
	...
	init_waitqueue_head(&hx280enc_data.waitq);
    ...
    np = of_find_compatible_node(NULL, NULL, "arkmicro,mae-enc");
    if (np) {
        hx280enc_clk = of_clk_get(np, 0);
        if (IS_ERR(hx280enc_clk)) {
            hx280enc_clk = NULL;
        } else {
            clk_prepare_enable(hx280enc_clk);
        }
    }
...
void __exit hx280enc_cleanup(void)
{
    ...
    if (hx280enc_clk) {
        clk_disable_unprepare(hx280enc_clk);
    }
