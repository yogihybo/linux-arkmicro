	...
	unsigned long arg)
{
	...
	switch (cmd) {
		...
		case MEMALLOC_IOCSFREEBUFFER:
			...
			spin_unlock(&fc->lock);
			break;
                case MEMALLOC_IOCSFLUSHRAMBUFFER:
                        spin_lock(&fc->lock);
                        if(__copy_from_user(&mem_params, (const void *) arg, sizeof(mem_params)))
                                return -EFAULT;

                        if (mem_params.busAddress && mem_params.size) {
                                outer_flush_range(mem_params.busAddress, mem_params.busAddress + mem_params.size);
                                ret = 0;
                        }
                        spin_unlock(&fc->lock);
                        //printk(KERN_ALERT "memalloc flush buffer: busAddress=0x%8x, size=0x%0x.\n",mem_params.busAddress, mem_params.size);
                        break;
		default:
			ret = -ENOIOCTLCMD;
	}
