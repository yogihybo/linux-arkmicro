...
static int ark1668_lcdfb_set_par(struct fb_info *info)
{
	...
	/* Read-modify-write, preserving yuv_order (bits[20:18], 3-bit) and
	 * rgb_order (bits[22:21], 2-bit) -- was a flat literal assignment
	 * that unconditionally zeroed them on every fb_set_par() call (every
	 * FBIOPUT_VSCREENINFO, including DirectFB's mode-set), clobbering
	 * whatever ark1668_lcdc_funcs.c's ark1668_lcdc_set_osd_format() (the
	 * ioctl path's own correct RMW helper) had set via the
	 * ARKFB_SET_WINDOW_FORMAT/atomic layer-fmt ioctls. Confirmed via
	 * Ghidra comparison against stock's real ark_disp_set_osd_format()
	 * (vmlinux.elf @ 0x802ddf98) as the root cause of skewed colors
	 * specifically on alpha-blended UI elements (2026-07-19) -- these
	 * fields control channel routing in the blend-unit's actual
	 * compositor math, which only runs for partial-alpha pixels; opaque
	 * pixels (alpha=255) pass through largely unaffected by a wrong
	 * value here, matching the reported symptom exactly (flat
	 * background fine, blended icons/widgets wrong). NOTE: field
	 * positions corrected 2026-07-19 -- yuv_order and rgb_order were
	 * initially swapped in this project's understanding (see
	 * ark1668_lcdc_set_osd_format() in ark1668_lcdc_funcs.c for the
	 * full trace); this RMW's mask happens to span both fields as one
	 * combined block so it was never wrong behaviorally, only the
	 * naming in this comment was backwards. bit16 (rgb_ycbcr_bypass) is
	 * intentionally cleared to 0 here, not preserved -- 0 is the
	 * format-correct value for RGBA888 per
	 * ark1668_lcdc_set_osd_format()'s own switch statement, not
	 * something the ioctl path should be setting independently. */
	value = lcdc_readl(sinfo, ARK1668_LCDC_OSD1_CTL);
	value &= (0x1F << 18);	/* keep only yuv_order/rgb_order (bits 18-22) */
