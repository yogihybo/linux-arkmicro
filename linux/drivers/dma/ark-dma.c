...
static int dw_dma_probe(struct dw_dma_chip *chip)
{
	...
	//dw_dma_off(dw); //dma is in use to play audio in u-boot
	...
	for (i = 0; i < pdata->nr_channels; i++) {
		...
		if (i != 0)
			channel_clear_bit(dw, CH_EN, dwc->mask); /* ch0 used in u-boot */
...
static int dw_dma_remove(struct dw_dma_chip *chip)
{
...
MODULE_AUTHOR("Sim");
MODULE_DESCRIPTION("Arkmicro dma driver");
