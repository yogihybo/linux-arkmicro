...
//static int es8316_read(u8 reg, u8 *rt_value, struct i2c_client *client)
//{
//	int ret;
//	u8 read_cmd[3] = {0};
//	u8 cmd_len = 0;
...
//		pr_err("es8316_read client->adapter==NULL\n");
//
//	ret = i2c_master_send(client, read_cmd, cmd_len);
//	if (ret != cmd_len) {
//		pr_err("es8316_read error1\n");
//		return -1;
//	}
//
//	ret = i2c_master_recv(client, rt_value, 1);
//	if (ret != 1) {
//		pr_err("es8316_read error2, ret = %d.\n", ret);
//		return -1;
//	}
//
//	return 0;
//}
//static int es8316_write(u8 reg, unsigned char value, struct i2c_client *client)
//{
//	int ret = 0;
//	u8 write_cmd[2] = {0};
//
...
//	if (ret != 2) {
//		pr_err("es8316_write error->[REG-0x%02x,val-0x%02x]\n",reg,value);
//		return -1;
//	}
...
static unsigned int es8316_read(struct snd_soc_component *component,
...
static void es8316_write(struct snd_soc_component *component,
			      ...
			      struct snd_soc_dai *dai)
{
	...
	if (playback) {//printk("startup:>>>>>>>>>>>>>>>>>>SNDRV_PCM_STREAM_PLAYBACK\n");
		...
		msleep(50);
	} else {//printk("startup:>>>>>>>>>>>>>>>>>>SNDRV_PCM_STREAM_CAPTURE\n");
...
static int es8316_init_regs(struct snd_soc_component *component)
{
	...
	snd_soc_component_write(component, ES8316_RESET_REG00, 0x30);
	msleep(30);
	...
	snd_soc_component_write(component, ES8316_ADC_PDN_LINSEL_REG22, 0x20);//LIN2MIC
	snd_soc_component_write(component, ES8316_ADC_PGAGAIN_REG23, 0xaa);
	...
	snd_soc_component_write(component, ES8316_ADC_MUTE_REG26, 0x20);
	...
	snd_soc_component_write(component, ES8316_SYS_PDN_REG0D, 0x1f);
	snd_soc_component_write(component, ES8316_SYS_LP1_REG0E, 0xff);
	snd_soc_component_write(component, ES8316_SYS_LP2_REG0F, 0xff);
	snd_soc_component_write(component, ES8316_RESET_REG00, 0xC0);
	snd_soc_component_write(component, ES8316_DAC_PDN_REG2F, 0x00);
	...
	snd_soc_component_write(component, ES8316_HPMIX_SWITCH_REG14, 0x88);
	snd_soc_component_write(component, ES8316_HPMIX_PDN_REG15, 0x88);
	snd_soc_component_write(component, ES8316_HPMIX_VOL_REG16, 0xBB);
	snd_soc_component_write(component, ES8316_CPHP_PDN2_REG1A, 0x10);
	snd_soc_component_write(component, ES8316_CPHP_LDOCTL_REG1B, 0x30);
	snd_soc_component_write(component, ES8316_CPHP_PDN1_REG19, 0x02);
	...
	snd_soc_component_write(component, ES8316_ADC_MUTE_REG26, 0x00);
	snd_soc_component_write(component, ES8316_RESET_REG00, 0x30);
	snd_soc_component_write(component, ES8316_CPHP_OUTEN_REG17, 0x66);

#else
	//update 20220330(ADC and DAC support)
	snd_soc_component_write(component, ES8316_RESET_REG00, 0x3f);
	usleep_range(5000, 5500);
	snd_soc_component_write(component, ES8316_RESET_REG00, 0x00);
	snd_soc_component_write(component, ES8316_SYS_VMIDSEL_REG0C, 0xFF);
	...
	snd_soc_component_write(component, ES8316_CAL_HPLIV_REG1E, 0x90);
	snd_soc_component_write(component, ES8316_CAL_HPRIV_REG1F, 0x90);
	snd_soc_component_write(component, ES8316_ADC_VOLUME_REG27, 0x11);//
	snd_soc_component_write(component, ES8316_ADC_PDN_LINSEL_REG22, 0x20);//LIN2MIC
	...
	snd_soc_component_write(component, ES8316_SDP_DACFMT_REG0B, 0x00);//0x01
	snd_soc_component_write(component, ES8316_SYS_VMIDLOW_REG10, 0x11);
	snd_soc_component_write(component, ES8316_SYS_VSEL_REG11, 0xFC);
	snd_soc_component_write(component, ES8316_SYS_REF_REG12, 0x28);
	snd_soc_component_write(component, ES8316_SYS_LP1_REG0E, 0x04);
	snd_soc_component_write(component, ES8316_SYS_LP2_REG0F, 0x0C);
	snd_soc_component_write(component, ES8316_DAC_PDN_REG2F, 0x00);//0x11
	snd_soc_component_write(component, ES8316_HPMIX_SEL_REG13, 0x00);
	snd_soc_component_write(component, ES8316_HPMIX_SWITCH_REG14, 0x88);
...
static int es8316_probe(struct snd_soc_component *component)
{
	...
	if (!ret) {
		...
		if (!ret) {
			...
			/* es8316_set_bias_level(component, SND_SOC_BIAS_OFF); */
			snd_soc_component_write(component, ES8316_CPHP_OUTEN_REG17, 0x66);//default:0x00	new:0x66
			snd_soc_component_write(component, ES8316_DAC_PDN_REG2F, 0x00);//default:0x11		new:0x00
			snd_soc_component_write(component, ES8316_CPHP_LDOCTL_REG1B, 0x30);//default:0x03	new:0x30
			...
			if (!es8316->hp_inserted)
				snd_soc_component_write(component, ES8316_SYS_PDN_REG0D,
					      0x1F);//default:0x3F		new:0x1F
			snd_soc_component_write(component, ES8316_SYS_LP1_REG0E, 0x3F);//default:0xFF		new:0x3F
			snd_soc_component_write(component, ES8316_SYS_LP2_REG0F, 0x1F);//default:0xFF		new:0x1F
			snd_soc_component_write(component, ES8316_CLKMGR_CLKSW_REG01, 0x7F);//default:0xF3		new:0x7F
			snd_soc_component_write(component, ES8316_ADC_PDN_LINSEL_REG22, 0x20);//default:0xc0	new:0x20
			//printk("end-----------------init regs\n");
		}
	}
...
static ssize_t es8316_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int val=0, flag=0;
	...
	if (flag) {
		...
		while(flag--) {
			es8316_write(es8316_component,reg, value_w);
		...
		do {
			value_r = 0;
			es8316_read(es8316_component,reg, &value_r);
...
static ssize_t es8316_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk("echo flag|reg|val > es8316\n");
	printk("eg read star addres=0x06,count 0x10:echo 0610 >es8316\n");
	printk("eg write star addres=0x90,value=0x3c,count=4:echo 4903c >es8316\n");
	//printk("eg write value:0xfe to address:0x06 :echo 106fe > es7243\n");
	return 0;
}

static DEVICE_ATTR(es8316, 0644, es8316_show, es8316_store);

static struct attribute *es8316_debug_attrs[] = {
	&dev_attr_es8316.attr,
	NULL,
};

static struct attribute_group es8316_debug_attr_group = {
	.name   = "es8316_debug",
	.attrs  = es8316_debug_attrs,
};
			    ...
			    const struct i2c_device_id *id)
{
	...
	*****/
	...
	ret = sysfs_create_group(&i2c->dev.kobj, &es8316_debug_attr_group);
	if (ret) {
