 ...
 * the VC uses.
 */
#define CONFIG_SYS_SDRAM_SIZE		SZ_64M //(SZ_128M + SZ_32M + SZ_16M + SZ_4M)
#define CONFIG_SYS_INIT_SP_ADDR		(CONFIG_SYS_SDRAM_BASE + CONFIG_SYS_SDRAM_SIZE - GENERATED_GBL_DATA_SIZE)
#define CONFIG_SYS_MALLOC_LEN		(CONFIG_ENV_SIZE+1024*1024)//0x80000

/* timer base address */
#define CONFIG_SYS_TIMERBASE		0x40405000

/* watchdog base address */
#define CONFIG_WATCHDOG_BASEADDR	0x40404000
 ...
 */
#define CONFIG_SYS_SPI_BASE			0x40402000
...
#define SFARGS \
	...
	"fdtupdatesd=fatload ${update_dev} ${dev_part} ${loadaddr} ark-cardvr.dtb; " \
...
#define NANDARGS \
	"mtdids=" CONFIG_MTDIDS_DEFAULT "\0" \
	"mtdparts=" CONFIG_MTDPARTS_DEFAULT "\0" \
	"bootstrapupdate=if fatload ${update_dev_type} ${update_dev_part} ${loadaddr} nboot.bin; " \
	...
	"bootloaderupdate=if fatload ${update_dev_type} ${update_dev_part} ${loadaddr} u-boot.bin; " \
...
#define CONFIG_SYS_SERIAL0      0x4040a000
#define CONFIG_SYS_SERIAL1      0x4040b000
