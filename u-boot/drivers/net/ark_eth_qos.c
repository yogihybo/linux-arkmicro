...
#define EQOS_MTL_TXQ0_OPERATION_MODE_TQS_MASK		0xf
...
#define EQOS_MTL_RXQ0_OPERATION_MODE_RQS_MASK		0x3f
#define EQOS_MTL_RXQ0_OPERATION_MODE_RFD_SHIFT		14
#define EQOS_MTL_RXQ0_OPERATION_MODE_RFD_MASK		0x1f
#define EQOS_MTL_RXQ0_OPERATION_MODE_RFA_SHIFT		8
#define EQOS_MTL_RXQ0_OPERATION_MODE_RFA_MASK		0x1f
...
#define EQOS_DMA_CH0_RX_CONTROL_RBSZ_SHIFT		5
#define EQOS_DMA_CH0_RX_CONTROL_RBSZ_MASK		0x3ff
...
static int eqos_start_clks_ark1668e(struct udevice *dev)
{
...
void eqos_stop_clks_ark1668e(struct udevice *dev)
{
	//struct eqos_priv *eqos = dev_get_priv(dev);
...
static int eqos_start_resets_ark1668e(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	int ret;
...
static int eqos_stop_resets_ark1668e(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
...
static ulong eqos_get_tick_clk_rate_ark1668e(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
...
static int eqos_set_tx_clk_speed_ark1668e(struct udevice *dev)
{
	//struct eqos_priv *eqos = dev_get_priv(dev);
...
static int eqos_adjust_link(struct udevice *dev)
{
	...
	ret = eqos_set_tx_clk_speed_ark1668e(dev);
	if (ret < 0) {
		pr_err("eqos_set_tx_clk_speed_ark1668e() failed: %d", ret);
		return ret;
	}
...
static int eqos_start(struct udevice *dev)
{
	...
	ret = eqos_start_clks_ark1668e(dev);
	if (ret < 0) {
		pr_err("eqos_start_clks_ark1668e() failed: %d", ret);
		goto err;
	}

	ret = eqos_start_resets_ark1668e(dev);
	if (ret < 0) {
		pr_err("eqos_start_resets_ark1668e() failed: %d", ret);
		goto err_stop_clks;
	}
	...
	rate = eqos_get_tick_clk_rate_ark1668e(dev);
	val = (rate / 1000000) - 1;
	writel(val, &eqos->mac_regs->us_tic_counter);

	for (i = 0; i < 8; i++) {
		eqos->phy = phy_connect(eqos->mii, i, dev, 0);
		if (eqos->phy)
			break;
	}
	if (!eqos->phy) {
...
err_shutdown_phy:
	...
	eqos->phy = NULL;
err_stop_resets:
	eqos_stop_resets_ark1668e(dev);
err_stop_clks:
	eqos_stop_clks_ark1668e(dev);
err:
	pr_err("FAILED: %d", ret);
	return ret;
}
...
static int eqos_probe_resources_ark1668e(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	int ret;
...
static int eqos_remove_resources_ark1668e(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
...
static int eqos_probe(struct udevice *dev)
{
	...
	ret = eqos_probe_resources_ark1668e(dev);
	if (ret < 0) {
		pr_err("eqos_probe_resources_ark1668e() failed: %d", ret);
...
err_remove_resources_tegra:
	eqos_remove_resources_ark1668e(dev);
...
static const struct eqos_config eqos_ark1668e_config = {
	.reg_access_always_ok = false,
};

static const struct udevice_id eqos_ids[] = {
	{
		.compatible = "arkmicro,ark1668e-eqos",
		.data = (ulong)&eqos_ark1668e_config
	},
	{ }
};
