...
static int do_update_rootfs (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	for(pos = 0,i = 1;pos < rootfsize;i++)
	{
		...
		printf("cmd = %s\n",cmd);
		run_command(cmd, 0);
		sprintf(cmd, "emmc write %s 0x%llx 0x%x",env_get("loadaddr"),off,file_size);
		printf("cmd = %s\n",cmd);
		run_command(cmd, 0);
		mdelay(100);
		sprintf(cmd, "emmc read %s 0x%llx 0x%x",env_get("cmploadaddr"),off,file_size);
		printf("cmd = %s\n",cmd);
		run_command(cmd, 0);
		mdelay(100);

		checksum = crc32(checksum, (const unsigned char *)dstdata, file_size);
		off = off + size;
		pos += size;
	...
	printf("cmd = %s\n",cmd);
	run_command(cmd, 0);
	printf(">>>rootfs crc value:0x%x\n",checksum);
	ret = ark_check_data_from_partition("rootfs",checksum);
	if(ret)
	{
		printf("checksum data error!!!\n");
		return 1;
	}
	return 0;
}
...
static int do_update_rootfs_from_ota(char *partition_name, char *file_name)
{
	...
	while(1)
	{
		...
		if(ret)
		{
			printf("Error:rootfs file maybe not in ota partition!!!\n");
			 return 2;
		}
		file_size = env_get_ulong("filesize", 16, 0x2000);
		if(file_size == 0)
	...
	printf("cmd = %s\n",cmd);
	run_command(cmd, 0);
	printf(">>>>ota rootfs crc value:0x%x\n",checksum);
	ret = ark_check_data_from_partition("rootfs",checksum);
	if(ret)
		return 1;
	return 0;
...
//	curr_partition  = env_get("updata_from_part");
	...
	printf("curr_partition %s,resetuboot = %d,update_fdt_name %s \n",curr_partition,resetuboot,update_fdt_name);
	mdelay(20);
