...
extern void get_Emmc_Parition_info(const char *filename,loff_t *off,loff_t *rtsize,loff_t *maxsize);
#endif
...
#ifdef CONFIG_CMD_EMMC
		get_Emmc_Parition_info(dev_part_str, &off,&rtsize,&maxsize);
#endif
		info->blksz = 512;
		info->start = off/info->blksz;//;0x877000;
		info->size = rtsize/info->blksz;//;0x619000;
		info->bootable = 0;
		printf("emmc %s info->start 0x%x++info->size 0x%x+++\n",dev_part_str,info->start,info->size);
		strcpy((char *)info->type, BOOT_PART_TYPE);
		strcpy((char *)info->name, dev_part_str);
		return 0;
	}
